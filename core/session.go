...
// the given native agent session ID. It prefers metadata in the current
// user/session key, then falls back to all sessions so /project grouping follows
// the native session across private chats and group chats.
func (sm *SessionManager) AgentSessionProject(userKey, agentSessionID string) (project, workDir string) {
	if agentSessionID == "" {
		return "", ""
	}
	if project, workDir := sm.agentSessionProjectForKeys(agentSessionID, []string{userKey}); project != "" {
		return project, workDir
	}
	return sm.agentSessionProjectForAllKeys(agentSessionID)
}

func (sm *SessionManager) agentSessionProjectForKeys(agentSessionID string, userKeys []string) (project, workDir string) {
	sm.mu.RLock()
	ids := make([]string, 0)
	...
	sm.mu.RUnlock()
	return sm.agentSessionProjectForSessionIDs(agentSessionID, ids)
}

func (sm *SessionManager) agentSessionProjectForAllKeys(agentSessionID string) (project, workDir string) {
	sm.mu.RLock()
...
func (sm *SessionManager) agentSessionProjectForSessionIDs(agentSessionID string, ids []string) (project, workDir string) {
